...
const parseBtnBlock = (btntxt) => {
    const normalizedStr = btntxt.split('\n').map(el => {
        const withoutCommentsEl = el.match(/^(.*?)\/\//gm)
        return withoutCommentsEl ? withoutCommentsEl[0].slice(0, -2) : el;
    })
    .join('\n')

    const parseActions = (actionsArr) => actionsArr.map(el => el.trim())
        .reduce((acc, el) => {
            const prefix = el.substring(0, 3)
            if (prefix === '{A}') {
                const [setName, actionName] = el.slice(4).split('|').map(el => el.trim())
    ...
    const btnDescription = normalizedStr.match(/[^\]]*$/gm).join()
        .split('%standart%')[0].trim();

    const standartActionsArr = normalizedStr.match(/%standart%([\s\S]*?)%expanded%/gm)[0].split('\n')
        .slice(1, -1)
    const standartActions = parseActions(standartActionsArr);

    const expandedActionsArr = normalizedStr.match(/%expanded%([\s\S])*$/gm)[0].split('\n')
        .slice(1)
...
const parse = (str) => {
    ...
    const buttonsArr = trimmedStr.split(/^(\[|\/\/\/)/gm)
        .reduce((acc, el, index, arr) => el === '[' || el === '///' ? [...acc, `${el}${arr[index + 1]}`] : acc, [])
        .map(el => el.trim())
